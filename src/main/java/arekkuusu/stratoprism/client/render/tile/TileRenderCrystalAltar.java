...
public class TileRenderCrystalAltar extends TileEntitySpecialRenderer<TileCrystalAltar> {
	...
	public void renderTileEntityAt(TileCrystalAltar te, double x, double y, double z, float partialTicks, int destroyStage) {
		...
		GlStateManager.translate(x + 0.5F, y + 0.95, z + 0.5);
		...
		if (te.hasWater()) {
			...
			if (items > 0) {
				...
				for (int i = 0; i < items; i++) {
					...
					float radiusX = (float) (radiusBase + 0.1 * Math.sin(ticks / modifier));
					float radiusZ = (float) (radiusBase + 0.1 * Math.cos(ticks / modifier));
	...
	public void renderWater(ResourceLocation loc) {
		...
		tessellator.getBuffer().pos(0, 16, 0).tex(par3Icon.getMinU(), par3Icon.getMaxV()).endVertex();
		tessellator.getBuffer().pos(16, 16, 0).tex(par3Icon.getMaxU(), par3Icon.getMaxV()).endVertex();
		tessellator.getBuffer().pos(16, 0, 0).tex(par3Icon.getMaxU(), par3Icon.getMinV()).endVertex();
	...
	public void renderCrystal(TileCrystalAltar te) {
		...
		Minecraft.getMinecraft().renderEngine.bindTexture(ResourceLocations.TEXTURE_CRYSTAL_BLUE);
		...
		MODEL_CRYSTAL.renderCrystal(CrystalVariant.CRYSTAL_RAW);
		//GlStateManager.disableBlend();
		GlStateManager.popMatrix();
	}
}
